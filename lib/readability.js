  ...
  try {
    ...
    function extractPublishedDateBasedOnAuthor(authorNodes) {
      ...
      for(var i = 0; i < authorNodes.length; i++) {
        possibleDates = possibleDates.concat( parsePossibleDateForNode(authorNodes[0].parentNode) );
        if(possibleDates.length > 0) break;
      }
      return dedupDates(possibleDates);
    }


    function dedupDates(allDates) {
      var dates = {};
      var date;
      for(var i = 0; i < allDates.length; i++) {
        date = allDates[i].date.format("YYYY-MM-DD");
        if ( !dates[ date ] )
          dates[ date ] = 0
        dates[ date ] += 1 - allDates[i].level  // penalize by the distance
      };
      ...
      for( date in dates ) {
        if(dates.hasOwnProperty(date))
          sortedDates.push({date: date, score: dates[date]});
      }
      sortedDates.sort(function(a,b) { return b.score - a.score });
    ...
    start(win, options, function(html) {
      ...
      callback({
        ...
        , publishedDate: publishedDate[0] ?  publishedDate[0].date : null
        , time: time / 1000
