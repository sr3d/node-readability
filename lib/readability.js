...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
    ...
    function extractImages(doc) {
      ...
      if (image) return [{url: normalizeUrl(image)}];
    ...
    function normalizeUrl(absOrRelativeUrl) {
    ...
    var AUTHOR_STOP_WORDS = /google|twitter|facebook|plus|rss|e.?mail|contact|view|website|subscribe|feed|continue|comment|follow|about/i;
    ...
    function extractAuthorWithMetaData(document) {
      ...
      for( var i = 0, len = metas._length; i < len; i++) {
        ...
        if ( AUTHOR_REGEX.test(attributes) && !BLACK_LISTED_AUTHOR_META.test(attributes) ) {
    ...
    function isPossiblyMetaAuthor(meta) {
      var attributes = [
        metas[i].getAttribute('property')
      ].join(" ").trim();
      if( /author/i.test(attributes) )
          return metas[i].getAttribute('content');
   ...
   // from Coffeescript
    ...
    function locateAuthorNode(node, possibleAuthor, level) {
      ...
      if ( typeof node.nodeValue != "undefined" && node.nodeValue != null) {
        ...
        if ( nodeValue.length > 5 && nodeValue.split(" ").length <= 5 
    ...
    */
    function dedupPossibleAuthorNodes(possibleAuthorNodes) {
      ...
      for(var author in authors) {
        ...
        if (/\band\b/.test(authors[author] ) )
          boost = 99;
        sortedAuthors.push( {author: author.trim(), nodes: authors[author], count: authors[author].length + boost});
        boost = 0
      }
      sortedAuthors.sort( function(a,b){ return b.count - a.count; })
    ...
    var POSSIBLE_DATE_REGEX = /publish|publishdate|lastModifiedDate|date|time|datePublished|displaydate/i,
        BLACK_LISTED_DATE_META_REGEX = /msvalidate/i,
                                  // fox news
        EXACT_DATE_META_REGEX = /dc\.date/i,
        POSSIBLE_DATE_VALUE = /\b20\d\d|\bJan|\bFeb|\bMar|\bApril|\bMay|\bJun|\bJul|\bAug|\bSep|\bOct|\bNov|\bDec|\bSun|\bMon|\bTue|\bWed|\bThu|\bFri|\bSat/i,
    ...
    function extractDateWithMetaData(document) {
      ...
      for(var date in possibleDates) {
        log('dates: ', date);
        sorted.push({date: date, count:possibleDates[date], parsed: tryParsingDate(date), algorithm: "meta" });
      }
    ...
    function tryParsingDate( possibleDate ) {
      ...
      try {
        ...
        if (/\d{6}/.test(possibleDate)) {
          publishedDate = moment(possibleDate, "YYYYMMDD");
        } else {
    ...
    function extractPublishedDateBasedOnAuthor(authorNodes) {
      ...
      log(" Total Possible Dates Found: ", possibleDates.length);
      log(possibleDates);
      ...
      while( ! found && parentNode ) {
        ...
        for(var i = 0, len = parentNode.childNodes._length; i < len; i++) {
          ...
          if ( isPossiblyDateNode(node) ) {
            ...
            log("isPossiblyDateNode", isPossiblyDateNode(node));
            log("POSSIBLE_DATE_VALUE", POSSIBLE_DATE_VALUE.test(nodeValue));


            possibleNodes = possibleNodes.concat( locateDateNode(node, nodeValue) );
            log(possibleNodes.length);
    ...
    function locateDateNode(node, possibleValue, level) {
      var found = [];
      var space = "";
      if(!level) level = 1;
      for(var i = 0; i < level; i++) {
      ...
      if ( node.nodeValue != null) {
        ...
        if ( POSSIBLE_DATE_VALUE.test(nodeValue)) {
          ...
          found.push( tryParsingDate(nodeValue) );
        }
      } else {
        var childNode;
        for (var i = 0, len = node.childNodes._length; i < len; i++) {
          ...
          if (POSSIBLE_DATE_VALUE.test(childContent) && childContent.length <= possibleValue.length ) {
            log( space + "looking into child node ", childNode.tagName);
            found = found.concat(locateDateNode(childNode, childContent, level + 1));
          }
        }
      }
    ...
    start(win, doc, options, function(html) {
      ...
      if (publishedDate) {
        log( "HAS PUBLISEHD DATE".red )
        data.publishedDate = publishedDate;
        data.date = publishedDate.date;
      }
      callback(data);
