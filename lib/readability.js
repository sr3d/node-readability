...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
    ...
    function extractAuthor(document) {
      ...
      // detect all possible author-looking nodes
      // only check for shorter text since author is most likely a name
      // then we dedup by calculate the overall density and grab the highest one
      for(var i = 0; i < allTags.length; i++) {
        tag = allTags[i];
        if( AUTHOR_REGEX.test(tag.getAttribute('name')) || 
            AUTHOR_REGEX.test(tag.getAttribute('class')) || 
            AUTHOR_REGEX.test(tag.getAttribute('itemprop')) ||
            AUTHOR_REGEX.test(tag.getAttribute('rel')) ) {
          possibleAuthor = readability.getInnerText(tag).trim();
          if (possibleAuthor && (possibleAuthor.length > 1 && possibleAuthor.length < 50 && possibleAuthor.split(" ").length <= 5)) {
            // console.log("possibleAuthor: ", possibleAuthor)
            if (typeof allAuthors[ possibleAuthor ] == "undefined") {
              allAuthors[ possibleAuthor ] = 0;
              authorNodes[ possibleAuthor ] = [];
            }
            allAuthors[ possibleAuthor ] += 1 + 1/i; // boosted by the pposition found
            authorNodes[ possibleAuthor].push(tag);
            authorCount++;
          }
        }
      }
      ...
      // find the most probably ones by compare the density ?
      for(var a in allAuthors) {
        if(allAuthors.hasOwnProperty(a) && a.length > 0 ) {
          // remove "By ..."
          a = a.replace(/^\bby\b/i,'').trim();
          authors.push({author: a, density: allAuthors[a] / authorCount, nodes: authorNodes[a] }); 
        }
      }
      // sort the highest density first
      authors.sort(function(a,b){ return b.density - a.density; })
    ...
    function tryParsingDate( possibleDate ) {
      ...
      try {
        ...
        publishedDate = date.isValid() ? date : null;
      } catch( ex ) {
        ; // don't do anything
        // console.log("exception:", ex.stack)
        // console.log("error parsing date: " + possibleDate);
      }
    ...
    var POSSIBLE_DATE_REGEX = /publish|lastModifiedDate|date/ig,
        POSSIBLE_DATE_FORMAT = /\b20\d\d|\bJan|\bFeb|\bMar|\bApril|\bMay|\bJun|\bJul|\bAug|\bSep|\bOct|\bNov|\bDec|\bSun|\bMon|\bTue|\bWed|\bThu|\bFri|\bSat/ig,
        HUMAN_DATE_FORMAT = /(\d+\s*\b\w+\b\s*ago)/i;


    /* Given a node, look at childNodes to parse out possible dates */
    function parsePossibleDateForNode(node, level) {
      if ( !node ) return [];
      if ( level > 2 ) return [];
      ...
      for(var i = 0; i < nodes.length; i++) {
        ...
        // we don't care on non-element node (e.g comment to text node)
        if(curNode.nodeType != NODE_TYPES.ELEMENT_NODE) continue;

        // attributes = [
        //   curNode.getAttribute('name')
        //   ,curNode.getAttribute('class')
        //   ,curNode.getAttribute('itemprop')
        // ].join(" ");

        // if (!POSSIBLE_DATE_REGEX.test(attributes)) {
        //   console.log("attributes:", attributes, " doesn't match")
        //   continue;
        // }

        value = readability.getInnerText(curNode).trim();

        // too long of a string, just bail out
        if( value.length > 100 ) {
          continue;
        }        

        if ( !POSSIBLE_DATE_FORMAT.test(value) ) {
          if ( HUMAN_DATE_FORMAT.test(value) )
            humanDate = RegExp.$1;

          if ( humanDate )
            ;// can try parsing here with datejs since datejs can parse this better

          continue;
        } 

        // console.log("FOUND SOMETHING - node value:", value)

        publishedDate = tryParsingDate(value);
        if(publishedDate) possibleDates.push({ date: publishedDate, level: level });
      }

      // console.log("possibleDates for level ", possibleDates, level)


      possibleDates = possibleDates.concat( parsePossibleDateForNode(node.parentNode, level + 1 ));
    ...
    function extractPublishedDateBasedOnAuthor(authorNodes) {
      ...
      for(var i = 0; i < authorNodes.length; i++) {
        ...
        // possibleDate = tryParsingDate(readability.getInnerText(node));
        // if (possibleDate) {
        //   console.log("found possible date right away:", possibleDates)
        //   possibleDates.push(possibleDate);
        // } else {
        //   // expand to parent node, basically we're doing a breadfirst search
        temp = possibleDates.concat( parsePossibleDateForNode(node.parentNode) );
        console.log("temp", temp)
          possibleDates = possibleDates.concat( parsePossibleDateForNode(node.parentNode) );
        // }

        if(possibleDates.length > 0) break;
      }
    ...
    function extractPublishedDateBasedOnDocument(document) {
      var publishedDate;
      var possibleDate;

      // microdata first 
      var regex = /publish[ed]/gi;
      var metas = document.getElementsByTagName("meta");
      for(var i = 0, len = metas.length; i < len; i++) {
        if ( regex.test(metas[i].getAttribute('itemprop')) ||
             regex.test(metas[i].getAttribute('name')) )
          possibleDate = tryParsingDate( metas[i].getAttribute('content') );

        if (possibleDate) 
          publishedDate = possibleDate
      }

      // if (possibleDate) publishedDate = tryParsingDate( possibleDate );
      if (publishedDate) return publishedDate;
        

      var tag, allTags = document.getElementsByTagName('*');
      var POSSIBLE_DATE_REGEX = /publish|lastModifiedDate|date/ig,
          POSSIBLE_DATE_FORMAT = /20\d\d|Jan|Feb|Mar|April|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|Sun|Mon|Tue|Wed|Thu|Fri|Sat/g,
          possibleDate, 
          matches = {},
          attributes;
      ...
      for(var i = 0; i < allTags.length; i++) {
        ...
        attributes = [
          ...
          // ,tag.getAttribute('datetime')
        ].join( " ");

        if( POSSIBLE_DATE_REGEX.test( attributes ) ) {
          value = readability.getInnerText(tag).trim();
          if (value.length < 60 && POSSIBLE_DATE_FORMAT.test(value))
            publishedDate = tryParsingDate(value);
        }
    ...
    readability.removeStyles(doc);


    var publishedDate, author;
