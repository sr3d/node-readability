...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
   ...
   // from Coffeescript
    ...
    var AUTHOR_REGEX = /byline|author/ig;
    var AUTHOR_STOP_WORDS = /google|twitter|plus|rss|e.?mail|contact|subscribe|feed/i;
    ...
    function locateAuthorNode(node, possibleAuthor, level) {
      var found = [];
      var space = "";
      if(!level) level = 1;
      ...
      if ( typeof node.nodeValue != "undefined" && node.nodeValue != null) {
        ...
        if ( nodeValue.length > 5 && nodeValue.split(" ").length <= 4 && !/[\d\@]/.test(nodeValue) && !AUTHOR_STOP_WORDS.test(nodeValue)) {
          log(space + "candidate added".green, nodeValue);
          found.push( {value: node.nodeValue, node: node});
        }
        ...
        for (var i = 0, len = node.childNodes._length; i < len; i++) {
          ...
          childContent = readability.getInnerText(node.childNodes[i]).trim();

          log( space + "content: ", childContent)

          if (childContent.length > 5 && childContent.length <= possibleAuthor.length ) {
            log( space + "looking into child node ", childNode.tagName);
            found = found.concat(locateAuthorNode(childNode, childContent, level + 1));
          }
        }
    ...
    /*
      possibleAuthors = [
        {value: "Potential Author Name", node: node },
      ]


      //output
      [
        { author: "", score: highest score },
      ]
    ...
    */
    function dedupPossibleAuthorNodes(possibleAuthorNodes) {
      ...
      for( var i = 0; i < possibleAuthorNodes.length; i++ ) {
        value = possibleAuthorNodes[i].value
        if( !authors[ value ] )
          authors[ value ] = [];
      ...
      for(var author in authors) {
        sortedAuthors.push( {author: author, nodes: authors[author], count: authors[author].length });
      }
    ...
    function parsePossibleDateForNode(node, level) {
      ...
      for(var i = 0, len = nodes._length; i < len; i++) {
        curNode = nodes[i];
        humanDate = null;
