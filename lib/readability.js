  ...
  var patch = {
    ...
    removeCommentNodes: function(document) {
      try {
        ...
        COMMENT_NODE_REGEX = /disqus|comment|archive|widget/i;
        POSSIBLE_CONTENT_NODE_REGEX = /content|post|entry/i;
...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
    ...
    function extractAuthor(document) {
      ...
      log("allNodes.length", allNodes.length)
    ...
    function tryParsingDate( possibleDate ) {
      ...
      try {
        ...
        if (/\d{6}/.test(possibleDate)) {
          ...
          publishedDate = date.isValid() ? date.format("YYYY-MM-DD") : null;
        }
