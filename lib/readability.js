...
exports.parse = function parse(theHtml, url, options, callback) {
  ...
  try {
    ...
    function extractAuthor(document) {
      ...
      // metas.concat(document.getElementsByTagName("span"))
      var itemProp;
      for(var i = 0, len = metas.length; i < len; i++) {
        itempProp = metas[i].getAttribute('itemprop');
        if (itemProp && itemProp.toUpperCase() == "author")
          author = metas[i].getAttribute('content');

        if (/author/i.test(metas[i].getAttribute("name")))
          author = metas[i].getAttribute('content');
      }
      ...
      for(var a in allAuthors) {
        if(allAuthors.hasOwnProperty(a) && a.length > 0 ) {
          authors.push({author: a, density: allAuthors[a] / authorCount }) 
        }
      }
      // sort the highest density first
      authors.sort(function(a,b){ return b.density - a.density; })
      
      author = authors[0] ? authors[0].author : null;
    ...
    start(win, options, function(html) {
      ...
      callback({title: document.title, content: html, images: images, author: author, publishedDate: publishedDate, time: time / 1000, inputLength: theHtml.length});
    });
